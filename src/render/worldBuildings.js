...
function getPlatformSeed(plat) {
  // Defensive: platforms should be objects, but avoid crashing if a bad entry sneaks in.
  if (!plat || (typeof plat !== "object" && typeof plat !== "function")) return 0;
...
function drawRoof(ctx, plat, seed, animTime, COLORS, state) {
  ...
  // top strip
  ctx.fillStyle = getColor(COLORS, "roofTop", "rgba(242,242,242,0.10)");
  ctx.fillRect(x, y, w, Math.min(3, h));

  // edge highlight
  ctx.fillStyle = getColor(COLORS, "platformEdge", "rgba(242,242,242,0.12)");
  ctx.fillRect(x, y, w, 2);
  ...
  if (crack01 > 0.01) {
    ...
    ctx.strokeStyle = crack01 > 0.75
      ? getColor(COLORS, "crackHi", "rgba(255,85,110,0.55)")
      : getColor(COLORS, "crack", "rgba(0,0,0,0.35)");
    ctx.lineCap = "round";
    ctx.lineJoin = "round";
  ...
  if (w > 140 && r > 0.35) {
    ctx.fillStyle = getColor(COLORS, "roofDetail", "rgba(242,242,242,0.10)");
    ...
    if (bodyH > 56 && bodyW > 80) {
      ...
      for (let y = startY; y + winH <= endY && row < maxRows; y += winH + gapY) {
        ...
        for (let x = startX; x + winW <= endX && col < maxCols; x += winW + gapX) {
          ...
          const onCol = getColor(COLORS, "windowOn", null) || `rgba(120,205,255,${(0.22 * dim).toFixed(3)})`;
          const offCol = getColor(COLORS, "windowOff", null) || `rgba(242,242,242,${(0.06 * dim).toFixed(3)})`;
